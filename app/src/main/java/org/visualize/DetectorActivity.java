...
import org.visualize.customview.OverlayView.DrawCallback;
import org.visualize.customview.OverlayView;
import org.visualize.env.BorderedText;
import org.visualize.env.ImageUtils;
import org.visualize.env.Logger;
import org.visualize.tflite.Classifier;
import org.visualize.tflite.TFLiteObjectDetectionAPIModel;
import org.visualize.tracking.MultiBoxTracker;
...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  public void startReading(String response) {
    textToSpeech = new TextToSpeech(getApplicationContext(), new TextToSpeech.OnInitListener() {
      ...
      public void onInit(int status) {
        if (status == TextToSpeech.SUCCESS) {
          ...
          int speech = textToSpeech.speak(response, TextToSpeech.QUEUE_FLUSH, null);
          Log.d("On beginning of speech", response);
        }
      }
    });
  }
          ...
          public void run() {
            ...
            for (final Classifier.Recognition result : results) {
              ...
              if (location != null && result.getConfidence() >= minimumConfidence &&
                ...
                float screenCenter = 150.f;
                Log.d("label", "screen center = " + screenCenter);
                Log.d("label", "previewWidth = " + previewWidth);
                float difference = screenCenter - objectCenter;
                Log.d("label", "" + difference);
                if(difference > 20){
                  // go to the left
